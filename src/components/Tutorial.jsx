...
export default function Tutorial({ step, onNext, onSkip, onAction, isWaiting }) {
  const s = TSTEPS[step];
  if(!s) return null;
    ...
    <div id="tutorial">
      <div id="tut-box">
        ...
        <div style={{display:"flex",gap:8}}>
          <button onPointerDown={(e)=>{e.stopPropagation();onSkip();}}
          ...
          {s.btn && (
            <button onPointerDown={(e)=>{e.stopPropagation();onAction(s);}}
          ...
          {!s.btn && (
            <div style={{flex:1,padding:10,textAlign:"center",color:"#3a5f82",fontSize:12,fontStyle:"italic"}}>
              Czekam aż wykonasz akcję...
            </div>
          )}
        </div>
      </div>
