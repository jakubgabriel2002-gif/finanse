...
const INIT_STATE = {
  ...
  fees: {rent:0,water:0,power:0},
  loan: null, elTmr: 48, riotOn: false, riotTmr: 0,
  inbox: [], events: [], news: [],
  nextUID: 100,
  buildMode: null, selUID: null, tab: "map",
  weather: WEATHERS[0],
...
function loadGame() {
  try {
    ...
    save.fees = save.fees || {rent:0,water:0,power:0};
    return save;
  } catch(e) { return null; }
}

export default function App() {
  const [G, setG] = useState(() => loadGame() || {...INIT_STATE});
  ...
  const [tutWaitTrigger, setTutWaitTrigger] = useState(0);
  ...
  // Initial recalc + reset tutorial if new game
  useEffect(() => {
    setG(g => ({...g, stats: calcStats(g.buildings, g.roads, g.loan, g.fees, g.weather), inbox: genInbox(g)}));
  }, []);
  ...
  // Recalc helper - returns new state
  ...
  // Month tick - 12s = 1 month at 1x speed
  useEffect(() => {
  ...
  const tileClick = useCallback((gx, gy) => {
    setG(prev => {
      ...
      if(prev.buildMode === 'road') {
        ...
        const next = recalc({...prev, roads: nr, budget: prev.budget-ROAD_COST, log: newLog});
        // Tutorial trigger
        if(!next.tutDone && next.tutStep === 1 && nr.size >= 3) {
          setTutWaitTrigger(t => t+1);
        }
        return next;
      }
      ...
      if(prev.buildMode) {
        ...
        const next = recalc({...prev, buildings, grid, budget: prev.budget-d.cost, nextUID: prev.nextUID+1, log: newLog});
  ...
  const upgradeBuilding = useCallback(() => {
    setG(prev => {
      ...
      const grid = {...prev.grid, [`${b.x},${b.y}`]: buildings.find(x=>x.uid===b.uid)};
      notif(`⬆️ ${d.e} ${d.n} → Lv${nl}`, "ok");
  ...
  const runAudit = useCallback(() => {
    setG(prev => {
      ...
      let extra = 0, msg = "🔍 Kontrola: wszystko w porządku.", logExtra = null;
      if(fs.length > 0 && Math.random() < 0.65) {
        ...
        logExtra = {id:logIdRef.current++,label:`💰 Kara — ${BD[caught.type].n}`,amount:fine};
      }
      notif(msg, "ok");
      let newLog = [{id:logIdRef.current++,label:"🔍 Kontrola skarbowa",amount:-500}, ...prev.log.slice(0,19)];
      if(logExtra) newLog.unshift(logExtra);
      setAuditModal(false);
  ...
  const tutAction = useCallback((step) => {
    if(step.action === 'finish') {
      setG(g => ({...g, tutDone: true, tutStep: TSTEPS.length}));
  ...
  const tutNext = useCallback(() => {
    setG(g => ({...g, tutStep: g.tutStep+1}));
  }, []);

  const tutSkip = useCallback(() => {
    setG(prev => {
      // Setup full starter city (skip tutorial)
      const sr = buildStarterRoads(CX, CY);
      const raw = [
        mk("townhall",CX,CY),mk("apartment",CX-1,CY+1),mk("apartment",CX+1,CY+1),
        mk("apartment",CX-1,CY-1),mk("apartment",CX+1,CY-1),mk("apartment",CX-2,CY),
        mk("factory",CX-4,CY),mk("shop",CX-1,CY+3),mk("shop",CX,CY+3),
        mk("hospital",CX-4,CY+3),mk("school",CX-4,CY-2),mk("park",CX+3,CY),
        mk("waterplant",CX+3,CY-2),mk("powerplant",CX-4,CY-4),
      ].filter(b => !sr.has(`${b.x},${b.y}`));
      const grid = Object.fromEntries(raw.map(b => [`${b.x},${b.y}`, b]));
      notif("🏙️ Witaj w NeoCity!", "ok");
      return recalc({...prev, budget: 80000, buildings: raw, grid, roads: sr, tutDone: true, tutStep: TSTEPS.length, nextUID: 200});
    });
  }, [notif, recalc]);

  // Tutorial waiting effect
  useEffect(() => {
    if(G.tutDone) return;
    const step = TSTEPS[G.tutStep];
    if(!step) return;
    if(step.waitForRoads && G.roads.size >= step.waitForRoads) {
      setTimeout(() => setG(g => ({...g, tutStep: g.tutStep+1})), 500);
    }
  }, [G.tutDone, G.tutStep, tutWaitTrigger, G.roads.size]);
  ...
  const onTouchEnd = useCallback((e) => {
    ...
    // Click handling is done via React onClick on tiles/buildings, not here
  }, []);
  ...
  return (
    <div id="app">
      ...
      <div id="main">
        {G.tab === 'map' ? (
          <div id="map-view" ref={mapViewRef}
            ...
            onWheel={onWheel}>
            <Map G={G} cam={cam} onTileClick={tileClick} onBldClick={bldClick} mapRef={mapRef}/>
      ...
      <div id="notifs">
        {notifs.map(n => (
          <div key={n.id} className="notif" style={{border:`1px solid ${n.type==='ok'?'rgba(0,232,122,0.5)':n.type==='err'?'rgba(255,61,90,0.5)':'rgba(255,215,0,0.5)'}`}}>
            {n.msg}
          </div>
        ))}
      </div>
      ...
      {/* Tutorial */}
      {!G.tutDone && <Tutorial step={G.tutStep} onNext={tutNext} onSkip={tutSkip} onAction={tutAction}/>}
      ...
      {auditModal && (
        <div className="modal-bg">
          <div className="mbox">
            ...
            <div style={{fontSize:12,color:"#6a90b8",marginBottom:12}}>Koszt: 500 zł. Szansa na wykrycie firm unikających podatków i nałożenie kary.</div>
            <div style={{display:"flex",gap:8}}>
